...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
	if (GPIO_Pin == BOARD1_DATA_READY_PIN)
	{
		...
		xSemaphoreGive(SPI_sem_req);//释放收到SPI信号量
...
void HAL_SPI_TxRxCpltCallback(SPI_HandleTypeDef *hspi)
{
	...
	if(current_rx_machine==0)//当前是第一块
	{
	  SCB_InvalidateDCache_by_Addr ((uint32_t *)aRxBuffer, BUFFERSIZE);
		BOARD1_CS_1();
		AD7606DataCounter = 0;
		for(j = 0; j < BUFFERSIZE; j+=16)
		{
			for(i=0;i<AD7606_ADCHS;i++)
			{
				aa_0 = aRxBuffer[2*i+j*16];
				aa_1 = aRxBuffer[2*i+1+j*16];
				aa_0_1 = ((aa_0<<8) | aa_1);
				y = aa_0_1;
				Ad7606_Data[current_rx_machine][AD7606DataCounter++]=y;
			
			}
		}
		if(AD7606DataCounter > BUFFERSIZE*2)
		{
			AD7606DataCounter = 0;
		}
		current_rx_machine = -1;
	...
	else if(current_rx_machine==1)//当前是第二块
	{
	  SCB_InvalidateDCache_by_Addr ((uint32_t *)aRxBuffer, BUFFERSIZE);
		...
		if(AD7606DataCounter > BUFFERSIZE*2-AD7606_ADCHS)
		{
			AD7606DataCounter = 0;
		}
			current_rx_machine = -1;
	...
	xSemaphoreGive(sem_spi_idle);//释放空闲SPI信号量
...
//	if(current_read_statue==board_all_read && AD7606DataCounter==(AD7606SAMPLEPOINTS*AD7606_ADCHS))
//	{
//		//这边就往上传了
//		current_read_statue = 0;
//		xSemaphoreGive(AD7606_ready);
//	}else if(current_read_statue==board_all_read && AD7606DataCounter>(AD7606SAMPLEPOINTS*AD7606_ADCHS*2-AD7606_ADCHS))
//	{
////		bsp_LedToggle(2);	
//		current_read_statue = 0;
